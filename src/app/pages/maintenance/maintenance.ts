...
@Component({
  ...
  template: `
    <main class="min-h-[82vh] bg-[#0D131A] text-[#F9F7F2] px-4 py-12 sm:py-20 flex items-center justify-center relative overflow-hidden">
      <!-- Ambient Glows -->
      <div class="absolute top-1/3 left-1/3 w-96 h-96 bg-[#00A7D4]/10 rounded-full blur-3xl pointer-events-none"></div>
      <div class="absolute bottom-1/4 right-1/4 w-96 h-96 bg-[#AE875B]/10 rounded-full blur-3xl pointer-events-none"></div>
      ...
      <div class="w-full max-w-2xl bg-[#151F2A] border border-[#AE875B]/30 rounded-3xl text-center px-6 py-12 md:px-12 md:py-16 shadow-2xl relative z-10 space-y-6">
        ...
        <!-- Maintenance Icon -->
        <div class="w-20 h-20 rounded-full bg-[#00A7D4]/10 border border-[#00A7D4]/35 mx-auto flex items-center justify-center text-[#00A7D4] shadow-inner">
          <mat-icon class="text-4xl w-10 h-10 flex items-center justify-center">construction</mat-icon>
        </div>
        ...
        <div class="space-y-2">
          <span class="inline-flex items-center gap-1.5 px-3.5 py-1 rounded-full text-xs font-bold uppercase tracking-wider bg-[#AE875B]/20 text-[#C9A87C] border border-[#AE875B]/35">
            <mat-icon class="text-xs">brush</mat-icon>
            GUAYABERAS ALUM
          </span>
        ...
        <p class="text-sm sm:text-base text-stone-300 leading-relaxed max-w-lg mx-auto font-sans">
          {{
            status.current()?.message
              || 'Nuestra tienda se encuentra temporalmente en mantenimiento para actualizar nuestro catálogo tekiteño. Regresa en unos minutos.'
          }}
        </p>
        ...
        <div class="pt-3 flex flex-col sm:flex-row justify-center items-center gap-4">
          ...
          >
            ...
            <span>{{ status.loading() ? 'Verificando...' : 'Intentar nuevamente' }}</span>
          </button>
          ...
          >
            <mat-icon class="text-sm">mail_outline</mat-icon>
            <span>Contacto</span>
          </a>
        </div>
      </div>
